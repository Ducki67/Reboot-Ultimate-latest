...
inline std::vector<PlayerBot> AllPawnBotsToTick;

namespace PawnBots
{
	static AController* SpawnPawnBot(FTransform SpawnTransform)
	{
		auto playerBot = PlayerBot();
		playerBot.Initialize(SpawnTransform);
		AllPawnBotsToTick.push_back(playerBot);
		return playerBot.Controller;
	}
}

extern UFortServerBotManagerAthena* BotManager;
